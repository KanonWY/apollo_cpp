...
CTcpClient::CTcpClient()
    : m_created(false), m_connected(false), m_async_request_in_progress(false)
{
}

CTcpClient::CTcpClient(const std::string &host_name_, unsigned short port_)
    : m_created(false), m_connected(false), m_async_request_in_progress(false)
{
    Create(host_name_, port_);
}

CTcpClient::~CTcpClient() { Destroy(); }
...
void CTcpClient::Create(const std::string &host_name_, unsigned short port_)
{
    ...
    try {
        ...
        m_async_worker = std::thread([this] { m_io_service->run(); });

        asio::connect(*m_socket,
                      resolver.resolve({host_name_, std::to_string(port_)}));
        // set TCP no delay, so Nagle's algorithm will not stuff multiple messages
        // in one TCP segment
...
bool CTcpClient::IsConnected() { return m_connected; }
...
size_t CTcpClient::ExecuteRequest(const std::string &request_, int timeout_,
...
void CTcpClient::ExecuteRequestAsync(const std::string &request_, int timeout_,
                                     AsyncCallbackT callback)
{
    ...
    if (!m_async_request_in_progress) {
        ...
        std::cerr << "CTcpClient::ExecuteRequestAsync failed: Another request is "
                     "already in progress"
                  << std::endl;
        ExecuteCallback(callback, "", false);
    }
...
size_t CTcpClient::ReceiveResponse(std::string &response_, int timeout_)
{
    try {
        ...
        // read stream header (sync)
            ...
            size_t bytes_read =
                m_socket->read_some(asio::buffer(&tcp_header, sizeof(tcp_header)));
            if (bytes_read != sizeof(tcp_header))
                read_failed = true;
            else
                read_done = true;
        ...
        do {
            ...
            size_t bytes_read =
                m_socket->read_some(asio::buffer(buffer, bytes_to_read));
            response_ += std::string(buffer, bytes_read);

            // std::cout << "CTcpClient::ReceiveResponse read response bytes " <<
            // bytes_read << " to " << response_.size() << std::endl;
        } while (response_.size() < rsize);

        return response_.size();
    } catch (std::exception &e) {
        std::cerr << "CTcpClient::ReceiveResponse: Failed to recieve response: "
                  << e.what() << std::endl;
        m_connected = false;
        return 0;
    }
                                      ...
                                      int /*timeout_*/)
{
    ...
    //      if (m_event_callback)
    //      {
    //        m_event_callback(client_event_timeout, "ReceiveResponseAsync
    //        timeouted");
    //      }
    ...
    m_socket->async_read_some(
        asio::buffer(tcp_header.get(), sizeof(tcp_header)),
        [this, tcp_header, callback_](
            ...
            if (bytes_transferred == sizeof(tcp_header)) {
                const auto resp_size =
                    static_cast<size_t>(ntohl(tcp_header->psize_n));
                this->ReceiveResponseData(resp_size, callback_);
            } else {
                std::cerr << "CTcpClient::ReceiveResponseAsync: Failed to receive "
                             "response: "
                          << "tcp_header size is invalid."
                          << "\n";
                this->ExecuteCallback(callback_, "", false);
            }
        });
...
void CTcpClient::ReceiveResponseData(const size_t size_,
                                     AsyncCallbackT callback_)
{
...
void CTcpClient::ExecuteCallback(AsyncCallbackT callback_,
                                 const std::string &data_, bool success_)
{
    m_async_request_in_progress = false;
...
std::vector<char> CTcpClient::pack_write(const std::string &request)
{
    ...
    memcpy(packed_response.data() + sizeof(tcp_header), request.data(),
           psize);
